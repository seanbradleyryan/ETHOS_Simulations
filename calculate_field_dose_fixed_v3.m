%% ETHOS IMRT Segment-by-Segment Dose Calculator (FIXED VERSION v3)
...
%   - Proper dual-layer MLC handling: Check BOTH MLC layers independently
%     * MLC1 (proximal): 28 leaf pairs (56 positions), boundaries -140 to 140 mm
%     * MLC2 (distal): 29 leaf pairs (58 positions), boundaries -145 to 145 mm (5mm offset)
%   - Corrected weight calculation: segment dose uses segmentWeight only
%     (dij is already scaled to beam meterset)
%   - BEAM ORDERING: Beams are sorted by gantry angle (ascending) to match
%     matRad's pln/stf structure ordering
...
%% ==================== HALCYON DUAL-LAYER MLC PARAMETERS ====================
% MLC1 (proximal layer): 28 leaf pairs (56 positions / 2)
% Boundaries: -140, -130, -120, ..., 130, 140 mm (10mm leaf width, 29 boundaries for 28 pairs)
MLC1_NUM_PAIRS = 28;
MLC1_LEAF_WIDTH = 10;  % mm
MLC1_BOUNDARY_START = -140;  % mm
MLC1_BOUNDARY_END = 140;  % mm
MLC1_BOUNDARIES = (MLC1_BOUNDARY_START : MLC1_LEAF_WIDTH : MLC1_BOUNDARY_END)';

% MLC2 (distal layer): 29 leaf pairs (58 positions / 2)
% Boundaries: -145, -135, -125, ..., 135, 145 mm (10mm leaf width, offset by 5mm from MLC1)
% 30 boundaries for 29 pairs
MLC2_NUM_PAIRS = 29;
MLC2_LEAF_WIDTH = 10;  % mm
MLC2_BOUNDARY_START = -145;  % mm (5mm offset from MLC1)
MLC2_BOUNDARY_END = 145;  % mm
MLC2_BOUNDARIES = (MLC2_BOUNDARY_START : MLC2_LEAF_WIDTH : MLC2_BOUNDARY_END)';

%% ==================== INITIALIZATION ====================
fprintf('==========================================================\n');
fprintf('  ETHOS IMRT Segment-by-Segment Dose Calculator\n');
fprintf('  FIXED VERSION v3 - Independent Dual-Layer MLC + Gantry Sort\n');
fprintf('==========================================================\n\n');
...
fprintf('Halcyon Dual-Layer MLC Configuration:\n');
fprintf('  - MLC1 (proximal): %d leaf pairs, boundaries [%.0f, %.0f] mm\n', ...
    MLC1_NUM_PAIRS, MLC1_BOUNDARY_START, MLC1_BOUNDARY_END);
fprintf('  - MLC2 (distal):   %d leaf pairs, boundaries [%.0f, %.0f] mm (5mm offset)\n', ...
    MLC2_NUM_PAIRS, MLC2_BOUNDARY_START, MLC2_BOUNDARY_END);
fprintf('  - Note: MLC positions arrays have 2*N elements (left bank + right bank)\n');
fprintf('\n');
...
for idxID = 1:length(ids)
    for idxSession = 1:length(sessions)
        ...
        %% Step 3: Extract segment information from RTPLAN with INDEPENDENT DUAL-LAYER MLC
        fprintf('\n[3/8] Extracting segment information from RTPLAN (independent dual-layer MLC)...\n');
        fprintf('  Note: Beams will be sorted by gantry angle to match matRad ordering\n');
        
        % Use a separate cache file for v3 segment data (sorted by gantry angle)
        segmentCacheFile = fullfile(dicomCacheDir, 'segmentData_v3_sorted.mat');
        ...
        if enableCaching && exist(segmentCacheFile, 'file')
            ...
            try
                ...
                for beamIdx = 1:numBeams
                    ...
                    % Store boundaries for both MLC layers
                    beamData.mlc1Boundaries = MLC1_BOUNDARIES;
                    beamData.mlc1NumPairs = MLC1_NUM_PAIRS;
                    beamData.mlc2Boundaries = MLC2_BOUNDARIES;
                    beamData.mlc2NumPairs = MLC2_NUM_PAIRS;
                    ...
                    % Detect dual-layer MLC from beam limiting device sequence
                    mlcDeviceNames = {};
                    ...
                    if isfield(beam, 'ControlPointSequence')
                        ...
                        for cpIdx = 1:numCP
                            ...
                            cpData.gantryAngle = [];
                            cpData.mlcLayer1 = [];  % First MLC layer (56 pairs)
                            cpData.mlcLayer2 = [];  % Second MLC layer (58 pairs)
                            cpData.jawX = [];
                            cpData.jawY = [];
                            ...
                            if isfield(cp, 'BeamLimitingDevicePositionSequence')
                                ...
                                for devIdx = 1:numDevices
                                    ...
                                    elseif contains(deviceType, 'MLC', 'IgnoreCase', true)
                                        % Handle dual-layer MLC - store each layer separately
                            ...
                            % Apply inherited values (NO COMBINATION - keep layers separate)
                        ...
                        % Create segments storing BOTH MLC layers separately
                        numSegments = numCP - 1;
                        validSegmentCount = 0;
                        ...
                        for segIdx = 1:numSegments
                            ...
                            % Store BOTH MLC layers separately (no combination)
                            segment.mlcLayer1 = cpStart.mlcLayer1;  % 56 leaf pairs
                            segment.mlcLayer2 = cpStart.mlcLayer2;  % 58 leaf pairs
                        ...
                        if beamIdx == 1 && debugMLC
                            ...
                            if ~isempty(beamData.segments)
                                seg = beamData.segments{1};
                                
                                % MLC Layer 1
                                if ~isempty(seg.mlcLayer1)
                                    numLeaves1 = length(seg.mlcLayer1) / 2;
                                    fprintf('      MLC Layer 1: %d leaf pairs\n', numLeaves1);
                                    fprintf('        Left bank range: [%.1f, %.1f] mm\n', ...
                                        min(seg.mlcLayer1(1:numLeaves1)), max(seg.mlcLayer1(1:numLeaves1)));
                                    fprintf('        Right bank range: [%.1f, %.1f] mm\n', ...
                                        min(seg.mlcLayer1(numLeaves1+1:end)), max(seg.mlcLayer1(numLeaves1+1:end)));
                                    midLeaf1 = round(numLeaves1/2);
                                    aperture1 = seg.mlcLayer1(numLeaves1 + midLeaf1) - seg.mlcLayer1(midLeaf1);
                                    fprintf('        Mid-leaf aperture: %.1f mm\n', aperture1);
                                else
                                    fprintf('      MLC Layer 1: NOT AVAILABLE\n');
                                end
                                
                                % MLC Layer 2
                                if ~isempty(seg.mlcLayer2)
                                    numLeaves2 = length(seg.mlcLayer2) / 2;
                                    fprintf('      MLC Layer 2: %d leaf pairs\n', numLeaves2);
                                    fprintf('        Left bank range: [%.1f, %.1f] mm\n', ...
                                        min(seg.mlcLayer2(1:numLeaves2)), max(seg.mlcLayer2(1:numLeaves2)));
                                    fprintf('        Right bank range: [%.1f, %.1f] mm\n', ...
                                        min(seg.mlcLayer2(numLeaves2+1:end)), max(seg.mlcLayer2(numLeaves2+1:end)));
                                    midLeaf2 = round(numLeaves2/2);
                                    aperture2 = seg.mlcLayer2(numLeaves2 + midLeaf2) - seg.mlcLayer2(midLeaf2);
                                    fprintf('        Mid-leaf aperture: %.1f mm\n', aperture2);
                                else
                                    fprintf('      MLC Layer 2: NOT AVAILABLE\n');
                                end
                                
                                if ~isempty(seg.jawX)
                                    fprintf('      Jaw X: [%.1f, %.1f] mm\n', seg.jawX(1), seg.jawX(2));
                                end
                                if ~isempty(seg.jawY)
                                    fprintf('      Jaw Y: [%.1f, %.1f] mm\n', seg.jawY(1), seg.jawY(2));
                                end
                            end
                        end
                    end
                    ...
                    fprintf('    Beam %d (RTPLAN order): %s, Gantry=%.1f°, %.1f MU, %d segments\n', ...
                        beamIdx, beamData.beamName, beamData.gantryAngle, beamData.beamMeterset, length(beamData.segments));
                end
                
                % CRITICAL: Sort beams by gantry angle to match matRad's pln/stf ordering
                fprintf('\n  Sorting beams by gantry angle to match matRad ordering...\n');
                
                % Extract gantry angles for sorting
                gantryAngles = zeros(numBeams, 1);
                for i = 1:numBeams
                    gantryAngles(i) = segmentData.beams{i}.gantryAngle;
                end
                
                % Sort by gantry angle (ascending)
                [sortedAngles, sortOrder] = sort(gantryAngles, 'ascend');
                
                % Reorder beams according to sorted gantry angles
                sortedBeams = cell(numBeams, 1);
                for i = 1:numBeams
                    sortedBeams{i} = segmentData.beams{sortOrder(i)};
                    % Update the beamIdx to reflect new ordering
                    sortedBeams{i}.originalRtplanIdx = sortedBeams{i}.beamIdx;
                    sortedBeams{i}.beamIdx = i;
                    
                    % Also update beamIdx in all segments
                    for segIdx = 1:length(sortedBeams{i}.segments)
                        sortedBeams{i}.segments{segIdx}.beamIdx = i;
                    end
                end
                
                % Replace with sorted beams
                segmentData.beams = sortedBeams;
                segmentData.sortOrder = sortOrder;  % Store mapping: sortOrder(new_idx) = old_idx
                
                % Display sorted order
                fprintf('  Beam order (sorted by gantry angle):\n');
                for i = 1:numBeams
                    fprintf('    New Beam %d: Gantry=%.1f° (was RTPLAN beam %d: %s)\n', ...
                        i, segmentData.beams{i}.gantryAngle, ...
                        segmentData.beams{i}.originalRtplanIdx, segmentData.beams{i}.beamName);
                end
        ...
        %% Step 6: Calculate segment-by-segment doses with INDEPENDENT DUAL-LAYER MLC
        fprintf('\n[6/8] Calculating segment-by-segment doses (independent dual-layer MLC)...\n');
        fprintf('  Note: Beams are now sorted by gantry angle to match stf ordering\n');
        ...
        for beamIdx = 1:length(stf)
            ...
            % Check for cached beam dose (use v3 cache file with sorted beams)
            beamDoseCacheFile = fullfile(patientCacheDir, sprintf('beam_%02d_dose_v3.mat', beamIdx));
            ...
            % Get MLC boundaries for this beam
            mlc1Boundaries = beamData.mlc1Boundaries;
            mlc1NumPairs = beamData.mlc1NumPairs;
            mlc2Boundaries = beamData.mlc2Boundaries;
            mlc2NumPairs = beamData.mlc2NumPairs;
            ...
            for segIdx = 1:numSegments
                ...
                mlcLayer1 = segment.mlcLayer1;  % 56 pairs
                mlcLayer2 = segment.mlcLayer2;  % 58 pairs
                jawX = segment.jawX;
                jawY = segment.jawY;
                
                segRaysInAperture = 0;
                
                % Check if we have both MLC layers
                hasMLC1 = ~isempty(mlcLayer1) && length(mlcLayer1) >= 2 * mlc1NumPairs;
                hasMLC2 = ~isempty(mlcLayer2) && length(mlcLayer2) >= 2 * mlc2NumPairs;
                
                if hasMLC1 || hasMLC2
                    % Extract left and right banks for each layer
                    if hasMLC1
                        leftBank1 = mlcLayer1(1:mlc1NumPairs);
                        rightBank1 = mlcLayer1(mlc1NumPairs+1:2*mlc1NumPairs);
                    end
                    if hasMLC2
                        leftBank2 = mlcLayer2(1:mlc2NumPairs);
                        rightBank2 = mlcLayer2(mlc2NumPairs+1:2*mlc2NumPairs);
                    end
                    ...
                    for rayIdx = 1:numRays
                        ...
                        % Check BOTH MLC layers independently
                        inMLC1 = true;  % Default to true if layer not present
                        inMLC2 = true;  % Default to true if layer not present
                        
                        if inJawX && inJawY
                            % Check MLC Layer 1 (56 pairs, boundaries -145 to 145)
                            if hasMLC1
                                inMLC1 = false;
                                for leafIdx = 1:mlc1NumPairs
                                    if leafIdx <= length(mlc1Boundaries)-1
                                        if rayPosY >= mlc1Boundaries(leafIdx) && rayPosY < mlc1Boundaries(leafIdx + 1)
                                            % Check if ray is within the aperture for this leaf pair
                                            if rayPosX >= leftBank1(leafIdx) && rayPosX <= rightBank1(leafIdx)
                                                inMLC1 = true;
                                            end
                                            break;  % Found the correct leaf pair
                                        end
                                    end
                                end
                            end
                            
                            % Check MLC Layer 2 (58 pairs, boundaries -150 to 150, offset by 5mm)
                            if hasMLC2
                                inMLC2 = false;
                                for leafIdx = 1:mlc2NumPairs
                                    if leafIdx <= length(mlc2Boundaries)-1
                                        if rayPosY >= mlc2Boundaries(leafIdx) && rayPosY < mlc2Boundaries(leafIdx + 1)
                                            % Check if ray is within the aperture for this leaf pair
                                            if rayPosX >= leftBank2(leafIdx) && rayPosX <= rightBank2(leafIdx)
                                                inMLC2 = true;
                                            end
                                            break;  % Found the correct leaf pair
                                        end
                        ...
                        % Ray must pass through BOTH MLC layers
                        inBothMLCs = inMLC1 && inMLC2;
                        
                        numEnergies = length(ray.energy);
                        for energyIdx = 1:numEnergies
                            bixelIdx = bixelIdx + 1;
                            if inBothMLCs
                                % FIX: Use only segment weight (dij is already scaled to beam meterset)
                    ...
                    % WARNING: No MLC data - this should NOT happen
                ...
                try
                    if any(w > 0)
                        ...
                        if verboseOutput
                            ...
                            fprintf('      Seg %d: Max=%.4f Gy, Weight=%.4f, Open=%.1f%% (%.2fs)\n', ...
                                segment.segmentIdx, maxDose, segment.segmentWeight, openFrac, calcTime);
                        end
                    else
                        if verboseOutput
