...
%   5. Load RTSTRUCT and create body/couch masks; save tissue_masks.mat
%   6. Process each dose file in batches with inline masking; save per-field .mat files
%   7. Load SCT images, sort by z-position, resample to dose grid, convert HU to density
%   8. Apply final mask to total_rs_dose; save total_rs_dose.mat, sct_resampled.mat, metadata.mat
%
%   NOTE: Masking is applied inline during field dose processing (step 7) to
%   avoid the memory cost of loading and re-saving each .mat file in a separate pass.
%   field_doses return value contains reference metadata (filepath, beam info) only;
%   full dose arrays must be loaded from individual .mat files if needed.
%
%   KEY TECHNICAL NOTES:
...
% Load masks BEFORE field dose processing so masking can be applied inline,
% avoiding a separate load/resave pass that doubles peak memory usage.

fprintf('\n[4/8] Loading RTSTRUCT and creating tissue classification masks...\n');
...
% Save tissue masks now (before dose processing, so they are on disk regardless of later errors)
...
% Precompute invalid_dose_mask once for use during inline field masking
if config.apply_dose_masking
    invalid_dose_mask = ~(body_mask & ~couch_mask);
    fprintf('  Inline masking ENABLED: %d voxels will be zeroed per field\n', sum(invalid_dose_mask(:)));
else
    invalid_dose_mask = false(ref_dims);
    fprintf('  Inline masking DISABLED (config.apply_dose_masking = false)\n');
end

%% ======================== PROCESS EACH FIELD DOSE ========================

fprintf('\n[5/8] Processing field doses with inline masking (batch_size=%d)...\n', config.batch_size);
...
for batch_idx = 1:num_batches
    ...
    for i = batch_start:batch_end
        ...
        try
            ...
            % Apply masking inline — avoids a separate load/resave pass after all files are saved
            if config.apply_dose_masking
            ...
            % Accumulate into batch subtotal (already masked)
...
fprintf('\n[6/8] Loading and resampling SCT to dose grid...\n');
...
%% ======================== FINALIZE TOTAL DOSE AND SAVE ========================

fprintf('\n[7/8] Finalizing total dose and saving...\n');

if config.apply_dose_masking
    % total_rs_dose is already masked (applied inline per field during step 6).
    % Re-apply to zero any floating-point accumulation residuals at invalid voxels.
    total_rs_dose(invalid_dose_mask) = 0;
    metadata.dose_masking_applied = true;
    fprintf('  Dose masking: ENABLED (applied inline during field processing)\n');
else
    metadata.dose_masking_applied = false;
    fprintf('  Dose masking: DISABLED (config.apply_dose_masking = false)\n');
end
...
fprintf('  Total dose max: %.4f Gy\n', metadata.total_dose_max_Gy);
fprintf('  Body voxels: %d\n', metadata.body_voxels);
fprintf('  Couch voxels: %d\n', metadata.couch_voxels);

save(fullfile(processed_dir, 'total_rs_dose.mat'), 'total_rs_dose', '-v7.3');
...
%% ======================== SAVE SCT RESAMPLED AND METADATA ========================
