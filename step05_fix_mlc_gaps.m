function [adjusted_paths, num_corrections] = step05_fix_mlc_gaps(patient_id, session, config)
%% STEP05_FIX_MLC_GAPS - Correct small MLC gaps in RTPLAN for Halcyon dual-layer MLC
%
%   [adjusted_paths, num_corrections] = step05_fix_mlc_gaps(patient_id, session, config)
%
%   PURPOSE:
%   Correct small MLC leaf gaps (<0.5mm) in both the REFERENCE and ADAPTED
%   RTPLAN files from ETHOS exports for Halcyon LINAC with dual-layer MLC.
%   Small gaps can cause RayStation import failures. This function expands
%   gaps symmetrically to minimum threshold while respecting leaf position
%   boundaries.
%
%   INPUTS:
...
%                                         (empty string if processing failed or file missing)
%       num_corrections  - Struct with fields:
%                            .reference - Integer, MLC gap corrections in reference plan
...
%       [paths, corrections] = step05_fix_mlc_gaps('1194203', 'Session_1', config);
%       % paths.reference, paths.adapted  -- output file paths
%       % corrections.reference, corrections.adapted -- number of corrections
%
%   DEPENDENCIES:
...
for pt = 1:length(plan_types)
    ...
    if ~isfile(input_filepath)
        warning('step05_fix_mlc_gaps:MissingPlan', ...
            '[%s] Input file not found: %s\n  Skipping this plan type.', ...
            upper(plan_type), input_filepath);
        continue;
    end
    ...
    if isfile(output_filepath)
        fprintf('  [INFO] Adjusted file already exists: %s\n', output_filename);
        fprintf('  [INFO] To reprocess, delete the existing file first.\n');
        adjusted_paths.(plan_type) = output_filepath;
        continue;
    end
    ...
    try
        rtplan = dicominfo(input_filepath);
    catch ME
        warning('step05_fix_mlc_gaps:DicomReadError', ...
            '[%s] Failed to read RTPLAN DICOM: %s\nError: %s', ...
            upper(plan_type), input_filepath, ME.message);
        continue;
    end

    % Verify this is an RTPLAN
    if ~isfield(rtplan, 'Modality') || ~strcmpi(rtplan.Modality, 'RTPLAN')
        warning('step05_fix_mlc_gaps:InvalidModality', ...
            '[%s] File is not an RTPLAN (Modality: %s). Skipping.', ...
            upper(plan_type), rtplan.Modality);
        continue;
    end

    % Verify BeamSequence exists
    if ~isfield(rtplan, 'BeamSequence')
        warning('step05_fix_mlc_gaps:NoBeamSequence', ...
            '[%s] RTPLAN does not contain BeamSequence. Skipping.', upper(plan_type));
        continue;
    end
            ...
            for cp_idx = 1:num_cp
                ...
                if cp_corrections > 0
                    ...
                    rtplan.BeamSequence.(beam_field).ControlPointSequence.(cp_field)...
                        .BeamLimitingDevicePositionSequence.(mlc_bld_field).LeafJawPositions ...
                        = corrected_positions;
    ...
    try
        dicomwrite([], output_filepath, rtplan, 'CreateMode', 'Copy');
        fprintf('    Write successful.\n');
    catch ME
        warning('step05_fix_mlc_gaps:DicomWriteError', ...
            '[%s] Failed to write adjusted RTPLAN: %s\nError: %s', ...
            upper(plan_type), output_filepath, ME.message);
        continue;
    end

    %% ---- Store results for this plan type ----

    adjusted_paths.(plan_type)    = output_filepath;
    num_corrections.(plan_type)   = total_corrections;
...
%
%   NOTE: This function is retained as a utility but is no longer used in
%   the main processing loop, which now relies on the standardised
%   RP_reference.dcm / RP_adapted.dcm naming from step0_sort_dicom.
    ...
    for i = 1:length(bld_fields)
        ...
        if contains(device_type, 'MLC', 'IgnoreCase', true)
            ...
            if isfield(device, 'NumberOfLeafJawPairs')
                ...
                mlc_info(mlc_count).num_pairs = 0;                   %#ok<AGROW>
            end
...
%       num_corrected          - Number of corrections made
    ...
    for i = 1:num_pairs
        ...
        if gap >= 0 && gap < min_gap
            new_bank_a = bank_a(i) - expansion;
            new_bank_b = bank_b(i) + expansion;
