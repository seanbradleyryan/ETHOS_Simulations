%% =========================================================================
%  RUN_DT_COMPARISON.m
%  dt convergence sweep: keep air physically correct at 343 m/s and coarsen the
%  k-Wave time step dt by a multiplier (1, 1.25, 1.5, 1.75, 2, 3, 4), reconstruct
%  each, and gamma-compare every coarsened run against the multiplier-1 recon.
%  Produces a pass-rate-vs-dt convergence curve.
%
%  Motivation: dt = cfl_number * min(spacing) / maxC sits at the CFL stability
%  limit. This sweep tests how far dt can be relaxed (fewer, larger time steps)
%  before the time-reversal reconstruction degrades. The total recording window
%  simTime = 2.5*gridDiag/minC is held fixed for every run, so Nt = ceil(simTime/dt)
%  shrinks as dt grows. The recently-added Nt_scaling (Nt/6 when air drives minC)
%  is applied to every run.
%
%  Modeled on run_air_sound_speed_comparison.m / run_standalone_simulation.m.
%  Noise is disabled for every run so any gamma difference is attributable to the
%  dt change alone.
%% =========================================================================
...
% runs is attributable to the dt change alone.
...
CONFIG.output_file  = 'dt_comparison_results.mat';
...
% --- dt convergence sweep. Air is fixed at the physically-correct 343 m/s for
%     every run. dt_base is the CFL-limited step; each run uses
%     dt = dt_base * multiplier. Multiplier 1 is the reference. Nt is recomputed
%     per run so the recording window simTime stays fixed. ---
CONFIG.air_sound_speed = 343;
CONFIG.dt_multipliers  = [1 1.25 1.5 1.75 2 3 4];

% --- Nt scaling (recently added to all simulations): air (~343 m/s) is the
%     slowest tissue, sets minC and inflates Nt. When air drives minC, divide
%     the recording length Nt by this factor. Applied to every run. ---
CONFIG.Nt_scaling = 6;
...
fprintf('=========================================================\n');
fprintf('  dt Convergence Sweep  (air fixed at %d m/s)\n', CONFIG.air_sound_speed);
fprintf('=========================================================\n');
...
fprintf('  dt multipliers:  [%s]\n', num2str(CONFIG.dt_multipliers));
fprintf('  Nt_scaling:      %g (Nt/%g when air drives minC)\n', CONFIG.Nt_scaling, CONFIG.Nt_scaling);
fprintf('=========================================================\n\n');
...
% Mask to body once (shared by all runs)
...
%% ========================= RUN dt SWEEP ================================
%  Air fixed at 343 m/s for every run. Each run uses dt = dt_base * multiplier
%  (dt_base = CFL step) and recomputes Nt for the fixed recording window; the
%  Nt_scaling (Nt/6) is applied inside simulate_and_reconstruct.

multipliers = CONFIG.dt_multipliers;
n_runs      = numel(multipliers);

for k = 1:n_runs
    mult_k = multipliers(k);
    fprintf('\n=========================================================\n');
    fprintf('  RUN %d/%d  -  dt multiplier = %.3g  (air = %d m/s)\n', ...
        k, n_runs, mult_k, CONFIG.air_sound_speed);
    fprintf('=========================================================\n');
    ...
    cfg.dt_multiplier = mult_k;

    out = simulate_and_reconstruct(cfg, sct, doseGrid, spacing_mm, fd_gantry_angle);
    out.dt_multiplier = mult_k;

    res(k) = out;

    fprintf('  [RUN %d] mult=%.3g -> dt=%.2e s, Nt=%d, minC=%.0f m/s, fwd=%.1fs, tr=%.1fs\n', ...
        k, mult_k, out.dt, out.Nt, out.minC, out.fwd_time, out.tr_time);
end

%% ========================= SPEEDUP SUMMARY ==============================

fprintf('\n========= dt SWEEP SUMMARY =========\n');
fprintf('  %-9s  %-10s  %-8s  %-10s  %-10s  %-10s\n', 'mult', 'dt(s)', 'Nt', 'fwd(s)', 'tr(s)', 'speedup');
t_ref_total = res(1).fwd_time + res(1).tr_time;
for k = 1:n_runs
    t_total = res(k).fwd_time + res(k).tr_time;
    fprintf('  %-9.3g  %-10.2e  %-8d  %-10.1f  %-10.1f  %-10.2fx\n', ...
        res(k).dt_multiplier, res(k).dt, res(k).Nt, res(k).fwd_time, res(k).tr_time, ...
        t_ref_total / max(t_total, eps));
end
fprintf('====================================\n');

%% ========================= IDENTIFY REFERENCE ==========================
% Reference = run 1 (multiplier 1, smallest dt) = ground truth. Every run is
% gamma-compared against it; the largest-dt run (last) is the most degraded.

recon_ref  = res(1).recon_dose;      % multiplier 1 (truth)
...
%% ========================= GAMMA ANALYSIS ===============================
%  Every run (each dt) vs the multiplier-1 recon. pass_rates is [n_runs x nCrit];
...
if exist('CalcGamma', 'file') == 2
    fprintf('\n[Gamma] Running dt-sweep gamma analysis (each dt vs multiplier-1 recon)...\n');
    ...
    % Keep gamma maps only for the reference-vs-largest-dt pair (for display).
    gamma_maps_largest = cell(nCrit, 1);
    ...
    for k = 1:n_runs
        ...
        fprintf('  [mult=%.3g, dt=%.2e, Nt=%d]\n', res(k).dt_multiplier, res(k).dt, res(k).Nt);
        for gc = 1:nCrit
            ...
            try
                ...
                if k == n_runs
                    gamma_maps_largest{gc} = gmap;
                end
            catch ME
                warning('Gamma [mult=%.3g, %s] failed: %s', res(k).dt_multiplier, lbl, ME.message);
                fprintf('  FAILED\n');
            end
        end
    end
    ...
    gamma_results.dt_list        = dt_list;
    gamma_results.Nt_list        = Nt_list;
    gamma_results.dt_multipliers = multipliers;
    gamma_results.cutoff_Gy      = low_dose_cutoff;
    gamma_results.eval_mask      = gamma_eval_mask;
    gamma_results.maps_largest   = gamma_maps_largest;  % ref vs largest-dt

    fprintf('\n  ------ Gamma Pass Rates vs multiplier-1 recon (10%% cutoff) ------\n');
    fprintf('  %-8s  %-10s  %-8s', 'mult', 'dt(s)', 'Nt');
    for gc = 1:nCrit, fprintf('  %-10s', gamma_criteria{gc, 3}); end
    fprintf('\n');
    for k = 1:n_runs
        fprintf('  %-8.3g  %-10.2e  %-8d', multipliers(k), dt_list(k), Nt_list(k));
        for gc = 1:nCrit
...
if CONFIG.save_results
    results = struct();
    results.recon_default  = recon_ref;                 % multiplier 1 (truth)
    results.recon_largest  = res(n_runs).recon_dose;    % largest dt
    results.air_sound_speed = CONFIG.air_sound_speed;
    results.dt_multipliers = multipliers;
    results.dt             = dt_list;
    results.Nt             = Nt_list;
...
if CONFIG.plot_results
    recon_largest = res(n_runs).recon_dose;

    % KEY DELIVERABLE: pass-rate-vs-dt convergence curve.
    if ~isempty(gamma_results) && isfield(gamma_results, 'pass_rates')
        plot_pass_rate_vs_dt(dt_list, multipliers, gamma_results.pass_rates, gamma_criteria);
    end
    ...
    % Recon comparison: multiplier 1 (top) vs largest dt (bottom).
    plot_dose_panels(recon_ref, recon_largest, sensor_msk, density_bg, spacing_mm, ...
        sprintf('Recon Dose: dt x%.3g (top) vs dt x%.3g (bottom)', multipliers(1), multipliers(end)), ...
        CONFIG.viz_smooth_sigma);

    % Gamma + absolute difference at the reference max-dose axial slice
    % (multiplier-1 vs largest-dt pair).
    if ~isempty(gamma_results) && isfield(gamma_results, 'maps_largest')
        gr_disp            = gamma_results;
        gr_disp.maps       = gamma_results.maps_largest;
        gr_disp.pass_rates = gamma_results.pass_rates(n_runs, :);
        gr_disp.mult_ref   = multipliers(1);
        gr_disp.mult_tgt   = multipliers(end);
        [~, max_idx]     = max(recon_ref(:));
        [~, ~, cz_gamma] = ind2sub(gridSize, max_idx);
        plot_gamma_and_error_axial(gr_disp, recon_ref, recon_largest, sensor_msk, cz_gamma);
    end
end

fprintf('\ndt convergence sweep complete.\n');
...
%SIMULATE_AND_RECONSTRUCT  Full forward + reconstruction pipeline for one dt
%  multiplier. Ported faithfully from run_standalone_simulation.m (medium ->
...
%  is applied. Returns recon_dose plus geometry and timing/step diagnostics.
    ...
    dt_base = CONFIG.cfl_number * min([dx, dy, dz]) / maxC;

    % dt convergence sweep: coarsen the time step by the multiplier. dt_base is
    % the CFL-limited step; larger multipliers relax the CFL condition.
    dt_mult = 1;
    if isfield(CONFIG, 'dt_multiplier') && ~isempty(CONFIG.dt_multiplier)
        dt_mult = CONFIG.dt_multiplier;
    end
    dt = dt_base * dt_mult;
    ...
    % Nt_scaling (recently added to all sims): air (~343 m/s) is the slowest
    % tissue, sets minC and inflates simTime/Nt. When air drives minC, shorten
    % the recording length by CONFIG.Nt_scaling (0 = disabled).
    Nt_prescale = Nt;
    ...
    kgrid.dt = dt;
    kgrid.Nt = Nt;
    fprintf('       dt = %.2e s (base %.2e x%.3g), Nt = %d (pre-scale %d), minC = %.0f m/s, T_sim = %.2e s\n', ...
        dt, dt_base, dt_mult, Nt, Nt_prescale, minC, Nt * dt);
...
%  gamma 10/10 | gamma 5/5 | gamma 3/3 | absolute dose difference.
    ...
    if isfield(gamma_results, 'mult_ref'), mult_ref = gamma_results.mult_ref; else, mult_ref = NaN; end
    if isfield(gamma_results, 'mult_tgt'), mult_tgt = gamma_results.mult_tgt; else, mult_tgt = NaN; end

    figure('Name', 'Gamma & Absolute Difference  Axial', 'Color', 'w', ...
        'NumberTitle', 'off', 'Position', [50, 300, 1400, 370]);
    sgtitle(sprintf('Axial Plane (Z = %d voxel)  Gamma (dt x%.3g vs dt x%.3g) & |Difference|', ...
        cz, mult_ref, mult_tgt), 'FontWeight', 'bold', 'FontSize', 11);
    ...
    hold(ax, 'off');
    xlabel(ax, 'X (voxel)'); ylabel(ax, 'Y (voxel)');
    title(ax, sprintf('|dt x%.3g - dt x%.3g|\nMax: %.4f Gy', mult_tgt, mult_ref, max_err));
    drawnow;
end


function plot_pass_rate_vs_dt(dt_list, multipliers, pass_rates, criteria)
%PLOT_PASS_RATE_VS_DT  Gamma pass-rate vs dt convergence curve.
%  dt_list     - [1 x nRuns] time steps (multiplier 1 = reference).
%  multipliers - [1 x nRuns] dt multipliers.
%  pass_rates  - [nRuns x nCrit] pass rates vs the multiplier-1 recon.
%  criteria    - {pct, dta, label} cell array, one row per gamma criterion.

    nCrit = size(criteria, 1);
    [mult_sorted, order] = sort(multipliers(:));   % ascending multiplier
    pr_sorted = pass_rates(order, :);
    dt_sorted = dt_list(order);

    figure('Name', 'Gamma Pass Rate vs dt', 'Color', 'w', ...
        'NumberTitle', 'off', 'Position', [80, 120, 760, 520]);
    hold on;
    markers = {'-o', '-s', '-^', '-d', '-v'};
    for gc = 1:nCrit
        m = markers{mod(gc - 1, numel(markers)) + 1};
        plot(mult_sorted, pr_sorted(:, gc), m, 'LineWidth', 1.8, ...
    ...
    grid on; box on;
    xlabel('dt multiplier (x CFL step)');
    ylabel('Gamma pass rate (%)');
    title(sprintf('Gamma Pass Rate vs dt  (reference = dt x%.3g = %.2e s)', ...
        mult_sorted(1), dt_sorted(1)));
