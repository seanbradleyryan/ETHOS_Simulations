function [recon_dose, sim_results] = run_second_field_simulation(field_dose, cbct_resampled, medium, medium_recon, beam_metadata, config, precomputed_sensor)
%RUN_SECOND_FIELD_SIMULATION k-Wave forward + BLIND time-reversal for one field
%
%   [recon_dose, sim_results] = run_second_field_simulation(field_dose, cbct_resampled, medium, medium_recon, beam_metadata, config, precomputed_sensor)
%
%   Blind-geometry counterpart to RUN_SINGLE_FIELD_SIMULATION. It models the
%   live IRAI situation where the acoustic waves are generated in and propagate
%   through the patient's TRUE (adapted, e.g. CT_3) anatomy, but the ONLY
%   geometry we experimentally have to invert on is the reference (e.g. CT_1)
%   anatomy. Concretely:
%
%       * FORWARD propagation (the waves that physically reach the detector) is
%         run on `medium` -- the TRUE/adapted geometry. The initial pressure
%         p0 = D * Gamma_true * rho_true is likewise built from `medium`.
%       * ITERATIVE TIME REVERSAL (and the residual re-forward inside the loop,
%         DAS delays, and the final pressure->dose conversion) is run on
%         `medium_recon` -- the reference geometry we blindly reconstruct on.
%
%   Both media are defined on the same base grid (both CBCTs are resampled onto
%   the dose grid), so every grid transform applied to `medium` for the forward
%   simulation -- force-uniform overrides, downscale, FFT-optimal padding, and
%   the determine_sensor_mask grid expansion / re-padding -- is applied to
%   `medium_recon` in lockstep, keeping the two voxel-aligned on the k-Wave grid.
%
%   Pass `medium_recon = []` (or omit it) to fall back to full-access behavior
%   (medium_recon == medium), i.e. identical to run_single_field_simulation.
%
%   INPUTS:
%       field_dose - Struct from step15_process_doses (the TRUE/adapted field
%           dose being simulated). Fields used:
...
%       cbct_resampled - Per-CBCT geometry struct for the TRUE anatomy the dose
%           was computed on (e.g. CBCT3_resampled from step15_process_doses).
%           Fields used here:
...
%       medium - FORWARD (TRUE-geometry) acoustic medium from
%           create_acoustic_medium() for the dose's own CBCT. Used to build p0
%           and to run the forward simulation. Struct fields:
...
%           .grid_size     - [nx, ny, nz]
%       medium_recon - RECONSTRUCTION (reference-geometry) acoustic medium from
%           create_acoustic_medium() for the reference CBCT (e.g. CBCT1). Used
%           for iterative time reversal, the residual re-forward inside the TR
%           loop, DAS delays, and the pressure->dose conversion. Same fields as
%           `medium` and the SAME base grid. Pass [] (or omit) for full access.
...
%       config - Configuration struct (see run_single_field_simulation for the
%           full list of honored fields; identical semantics here).
%       precomputed_sensor - (Optional) Plan-level sensor mask computed ONCE by
%           the caller (pipeline_simulate) and reused for every field. Honored
%           ONLY for sensor_placement_method='determine_sensor_mask'. Because
%           the sensor mask must be IDENTICAL for the forward and reconstruction
%           passes, using a single precomputed plan mask (built from the
%           reference CT) is the recommended path here. Pass [] or omit to
%           compute the mask inline.
%
%   OUTPUTS:
%       recon_dose  - 3D reconstructed dose array (Gy), cropped back to the
%                     input field_dose.dose_Gy size (or the downscaled size).
%                     The dose is recovered on the RECONSTRUCTION geometry.
%       sim_results - Struct with diagnostics (see run_single_field_simulation),
%                     plus .blind_recon (logical) flagging that the forward and
%                     reconstruction media differed.
%
%   NOTES:
...
%       - The forward medium and the reconstruction medium MUST share the base
%         grid; a size mismatch is an error.
%
%   AUTHOR: ETHOS Pipeline Team
%   DATE: July 2026
%   VERSION: 1.0 (blind-geometry variant of run_single_field_simulation 3.0:
%                 forward on true medium, time reversal on reference medium)
%
%   See also: run_single_field_simulation, run_standalone_comparison,
%             determine_sensor_mask, kspaceFirstOrder3D
    ...
    if nargin < 5, beam_metadata = []; end
    if nargin < 7, precomputed_sensor = []; end

    % Blind reconstruction: when a distinct reconstruction medium is supplied,
    % forward-propagate on `medium` (true geometry) but time-reverse on
    % `medium_recon` (reference geometry). Missing/empty -> full access.
    if nargin < 4 || isempty(medium_recon)
        medium_recon = medium;
        blind_recon  = false;
    else
        blind_recon  = true;
    ...
    if ~isequal(gridSize, medium.grid_size)
        error('run_second_field_simulation:SizeMismatch', ...
            'Field dose [%d %d %d] does not match forward medium [%d %d %d].', ...
            Nx, Ny, Nz, ...
    ...
    if blind_recon && isfield(medium_recon, 'grid_size') && ...
        ...
        error('run_second_field_simulation:ReconSizeMismatch', ...
    ...
    if isempty(meterset) || meterset <= 0
        warning('run_second_field_simulation:NoMeterset', ...
    ...
    if blind_recon
        ...
        fprintf('        [FULL ACCESS] Forward and reconstruction on the same medium.\n');
    end
    ...
    %  with uniform values. Applied to BOTH the forward and reconstruction media
    %  so the two stay comparable under the override.

    if force_uniform_density
        medium.density       = ones(gridSize) * uniform_density_val;
        medium_recon.density = ones(gridSize) * uniform_density_val;
        fprintf('        [Override] density forced uniform = %g kg/m^3\n', uniform_density_val);
    end
    if force_uniform_speed
        medium.sound_speed       = ones(gridSize) * uniform_speed_val;
        medium_recon.sound_speed = ones(gridSize) * uniform_speed_val;
    ...
    if force_uniform_atten
        ...
        medium_recon.alpha_coeff = ones(gridSize) * uniform_alpha_coeff_val;
    ...
    if force_uniform_gruneisen
        ...
        medium_recon.gruneisen = ones(gridSize) * uniform_gruneisen_val;
        fprintf('        [Override] gruneisen forced uniform = %g\n', uniform_gruneisen_val);
    end

    %% ======================== DOWNSCALE FACTOR ========================
    %  Optional pre-sim downscale of the whole grid (dose, both media, body/couch
    %  masks). Applied identically to the forward and reconstruction media.
    ...
    %% ======================== INITIAL PRESSURE p0 ========================
    %  Built on the FORWARD (true) medium: the physical p0 is generated in the
    %  true anatomy (p0 = D/N_pulses * Gamma_true * rho_true).
    ...
    if ~any(doseMask(:)) || max(p0(:)) == 0
        warning('run_second_field_simulation:NoDose', ...
    ...
    %  Pad grid to FFT-friendly dimensions for k-Wave performance. Both media
    %  (and p0) are padded identically; padding region filled with water.

    Nx_orig       = Nx;
    Ny_orig       = Ny;
    Nz_orig       = Nz;
    gridSize_orig = gridSize;
    medium_orig       = medium;         % forward medium at orig size (pre-FFT-pad)
    medium_recon_orig = medium_recon;   % reconstruction medium at orig size
    ...
    %  re-run FFT-optimal padding, which changes Nx/Ny/Nz. The SAME sensor mask
    %  is used for the forward and reconstruction passes.
    ...
    switch sensor_method
        ...
        case 'box'
            ...
            if bx_hi_x <= bx_lo || bx_hi_y <= bx_lo || bx_hi_z <= bx_lo
                error('run_second_field_simulation:BoxTooSmall', ...
        ...
        case 'determine_sensor_mask'
            ...
            if isfield(sensor_info_orig, 'grid_pad') && sensor_info_orig.grid_pad.expanded
                ...
                % medium_orig drives per-plot density backgrounds; update it to
                % the expanded-but-unpadded forward medium.
                medium_orig = struct( ...
                    'density',     density_exp, ...
                ...
                % Mirror the strip->water-expand onto the reconstruction medium
                % so it occupies the identical expanded grid. Nx_orig/Ny_orig/
                % Nz_orig are still the PRE-expansion sizes here (bumped below).
                % medium_recon_orig drives the pressure->dose conversion, so it
                % must track the expanded-but-unpadded reconstruction medium.
        ...
        otherwise
            error('run_second_field_simulation:UnknownSensorMethod', ...
    ...
    if numSensorPts == 0
        warning('run_second_field_simulation:EmptySensor', ...
    ...
    if plot_results
        try
            ...
            warning('run_second_field_simulation:PreSimPlotFail', ...
    ...
    % propagation (true medium) and the reconstruction propagation (reference
    % medium), so bound the step by the fastest tissue across BOTH media and set
    % the recording length by the slowest across both. With the shared tissue
    % tables these are identical (bone 3200 / air 343), but taking the extrema
    % over both keeps the recon pass CFL-stable if the media ever diverge.
    maxC = max(max(medium.sound_speed(:)), max(medium_recon.sound_speed(:)));
    minC_fwd = min(medium.sound_speed(medium.sound_speed > 0));
    minC_rec = min(medium_recon.sound_speed(medium_recon.sound_speed > 0));
    minC = min(minC_fwd, minC_rec);
    ...
    %% ======================== k-WAVE MEDIA ========================
    %  kmedium     : FORWARD (true) medium -- generates the sensor data.
    %  kmedium_rec : RECONSTRUCTION (reference) medium -- time reversal inverts
    %                on this. Identical to kmedium for full-access doses.
    ...
    %% ======================== FORWARD SIMULATION ========================
    %  Runs on the FORWARD (true) medium: the waves the detector would actually
    %  measure given the true anatomy.

    fprintf('        Running forward simulation (true medium)...\n');
    ...
    try
        ...
        warning('run_second_field_simulation:ForwardFail', ...
    ...
    %% ======================== RECONSTRUCTION ========================
    %  Runs on the RECONSTRUCTION (reference) medium: iterative time reversal,
    %  the residual re-forward inside the loop, and DAS delays all use
    %  kmedium_rec / medium_recon -- the blind geometry we invert on.
    ...
    switch recon_method
        case 'tr'
            ...
            if plot_results
                try
                    ...
                    warning('run_second_field_simulation:LiveFigFail', ...
                        'Live TR figure setup failed: %s', ME.message);
                    fig_live = [];
                end
            end

            fprintf('        Running iterative time reversal on reference medium (%d iterations, tol=%.1e)...\n', ...
            ...
            try
                ...
                for tr_iter = 1:num_tr_iter
                    ...
                    % Residual correction for next iteration (on reference medium)
                ...
                warning('run_second_field_simulation:TRFail', ...
        ...
        case 'das'
            fprintf('        Running Delay-And-Sum reconstruction (reference medium)...\n');
            try
                das_tic = tic;
                das_opts = struct( ...
                ...
                reconPressure = das_reconstruct(sensorData, sensor, sensor_info, medium_recon, ...
                ...
                warning('run_second_field_simulation:DASFail', ...
        ...
        otherwise
            error('run_second_field_simulation:UnknownReconMethod', ...
    ...
    %  reconstruction gruneisen/density).
    ...
    if use_pressure_scale_corr
        ...
        if recon_max > 0
            ...
            warning('run_second_field_simulation:RecoMaxZero', ...
    ...
    %  Blind reconstruction: the recovered pressure lives on the RECONSTRUCTION
    %  (reference) geometry, so convert pressure->dose with the reconstruction
    %  medium's gruneisen/density -- the blind map we experimentally have. The
    %  body mask still comes from the true anatomy (cbct_resampled).
    ...
    if ~isequal(size(recon_dose), input_dims_for_crop)
        ...
        try
            ...
            warning('run_second_field_simulation:CropFail', ...
    ...
    if plot_results
        ...
        % The density background is the RECONSTRUCTION geometry (the anatomy the
        % recovered dose lives on).
        eo  = expansion_offsets;
        idc = input_dims_for_crop;
        try
            sensor_for_plot = sensor.mask(eo(1) + (1:idc(1)), ...
                                          eo(2) + (1:idc(2)), ...
                                          eo(3) + (1:idc(3)));
            density_for_plot = medium_recon_orig.density(eo(1) + (1:idc(1)), ...
                                                         eo(2) + (1:idc(2)), ...
                                                         eo(3) + (1:idc(3)));
            plot_dose_panels(doseGrid, recon_dose, sensor_for_plot, ...
                density_for_plot, spacing_mm, ...
                'Dose Comparison: True Dose vs Blind Reconstruction');
        catch ME
            warning('run_second_field_simulation:DosePanelsFail', ...
        ...
        if strcmpi(recon_method, 'tr')
            try
                ...
                warning('run_second_field_simulation:ConvPlotFail', ...
                    'plot_convergence_history failed: %s', ME.message);
            end
        end
    end

    fprintf('        Second-field (blind) simulation complete.\n');
end
...
%  sensor mask as red contour.
    ...
    rows = { ...
        struct('label', 'True dose',          'data', original), ...
        struct('label', 'Blind reconstruction','data', recon)};
